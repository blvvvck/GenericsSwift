...
class ViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, DataTransferProtocol, UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    func initNews() {
        guard let currentNews: [News] = newsManager.syncGetAll() else { return }
        news = currentNews
    }
    ...
    func randomNews() {
        ...
        let news1 = News(text: nil, image: newsImageArray[0], likesCount: newsLikesArray[0], commentsCount:newsCommentsArray[0], repostsCount: newsRepostsArray[0], name: nameLabel.text!, surname: surnameLabel.text!, date: newsDateArray[Int(arc4random_uniform(UInt32(newsDateArray.count)))], id: UUID().uuidString )
        ...
        newsManager.syncSave(with: news1)
        newsManager.syncSave(with: news2)
        newsManager.syncSave(with: news3)
    ...
    func didPressDone(with note: String) {
        ...
        newsManager.asyncSave(with: tempNews) {[weak self] (isSaved) in
            ...
            if (isSaved) {
                strongSelf.newsManager.asyncGetAll(completionBlock: { (news: [News]) in
